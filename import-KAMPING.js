...
// Генерираме уникално име за теста, за да не се блъска в старите опити
const UNIQUE_ID = Math.floor(Math.random() * 9999);
const TARGET_SKU = `NEW_${UNIQUE_ID}`; 
...
const FILENAME = `${TARGET_SKU}.jpg`;

async function run() {
    try {
        console.log(`1. 📦 Създаване на продукт със SKU: ${TARGET_SKU}...`);
        const pRes = await fetch(`https://${SHOPIFY_DOMAIN}/admin/api/${API_VERSION}/graphql.json`, {
            method: 'POST',
            headers: { 'X-Shopify-Access-Token': ACCESS_TOKEN, 'Content-Type': 'application/json' },
            body: JSON.stringify({ query: `mutation { productCreate(input: { title: "Test ${TARGET_SKU}" }) { product { id } } }` })
        });
        ...
        console.log(`3. 🔍 Резервиране на Staged Upload за ${FILENAME}...`);
        ...
        console.log(`\n✨ ГОТОВО. Виж в админа продукта с име "Test ${TARGET_SKU}".`);
    } catch (err) {
